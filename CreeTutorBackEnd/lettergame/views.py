...
from .models import Alphabet, LettergameStats, LetterPairs, PairletterStats
import random
import datetime
...
def letterpair(request):
    ...
    '''
    Function takes in request.
    If request.method is GET, then function queries letters from the table LetterPairs,
    randomizes their order, takes the first five, and passes it into dictionary "context". The sound for the exercise
    is randomly chosen from these chosen 5.
    ...
    if request.method == 'GET':
        letters = sorted(LetterPairs.objects.all().order_by('name'), key=lambda x: random.random())
        letters = letters[:5]
        sound = random.choice(letters)
        context = {
