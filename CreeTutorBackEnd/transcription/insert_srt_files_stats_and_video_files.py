"""
File contains code that grabs stats srt files for the video in the specified folder, then adds the stats and the
location of the files to the db.

This file looks similar to insert_srt_files_stats_and_audio_files because it is. Due to the fact they perform the same
...
if __name__ == '__main__':
    ...
    for file in os.listdir(directory_path):
        if not file.startswith('.'):
            ...
            VideoAndSubtitleFilesForTranscription.objects.get_or_create(dict_for_insert, name=dict_for_insert['name'])